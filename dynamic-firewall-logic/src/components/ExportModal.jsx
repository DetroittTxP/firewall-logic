...
export function ExportModal({ modal, modalTab, setModalTab, onClose }) {
  if (!modal) return null;

  const content =
    modalTab === 'readable' ? modal.readable
    : modalTab === 'pseudo'  ? (modal.pseudo || '# (no connected flow to export)')
    : (modal.yaml || '# (no connected flow to export)');
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <span style={{ color: '#09090b', fontWeight: 'bold', fontSize: '14px' }}>📋 Logic Export</span>
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', color: '#a1a1aa', fontSize: '20px', cursor: 'pointer', lineHeight: 1 }}
          >×</button>
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', borderBottom: '1px solid #e4e4e7' }}>
          {[['readable', '📖 Human Readable'], ['pseudo', '💻 Pseudocode'], ['yaml', '📄 YAML']].map(([key, label]) => (
            <button
              key={key}
              ...
              style={{
                ...
                fontSize: '12px', borderBottom: modalTab === key ? '2px solid #18181b' : '2px solid transparent',
              }}
            >
              {label}
            </button>
        ...
        {/* Footer */}
        <div style={{
          padding: '10px 18px', borderTop: '1px solid #e4e4e7',
          ...
          <button
            ...
            style={{
              ...
              padding: '7px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px',
            }}
          >
            📋 Copy
          </button>
